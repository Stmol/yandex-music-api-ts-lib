...
export interface SimilarEntityDataShape extends Record<string, unknown> {
  album?: Album | null;
  artist?: Artist | null;
...
export class SimilarEntityData {
  ...
  constructor(shape: SimilarEntityDataShape) {
    assignModelShape(this, shape);
  }

  static fromJSON<TModel extends SimilarEntityData>(
    this: new (shape: SimilarEntityDataShape) => TModel,
    json: DeepReadonly<JsonObject>,
  ): TModel {
    const normalized = normalizeObject(json) as Record<string, JsonValue>;
    const shape: SimilarEntityDataShape = { ...normalized };
